# SeaForge SpeedRun Cut · 55 seconds

A beat-by-beat shooting script for the live demo and the fallback recording.
Mirrors the locked spine in `SeaForge_Sequencing_Plan_v2.md` ("vessel goes
dark → second identity → preserve hypotheses → refuse → review rule changes
the next case"). Practice 5+ times before stage; the make-or-break beat is
the second-case recommendation change.
...
| 0:29 | 6 s | 6. The Intent refusal | "The Intent specialist refuses. The system will not infer hostile intent from this evidence." | RefusalCard prominent · "EPISTEMIC GUARD" tag · bounded-reads list. |
...
| 0:53 | 2 s | 10. Tagline | "SeaForge: contested artifacts, honest refusals, prior rules that travel." | Brand bar in topbar held on screen. |

Total: 55 s.

## Fallback recording
- Record the same beat sequence at 1× speed using QuickTime / OBS · save as
  `docs/seaforge-speedrun-55s.mp4`.
...
- Beat 6 (Intent refusal) and beat 8 (second-case changed) are non-negotiable.
  Per `SeaForge_Sequencing_Plan_v2.md`: "A second event card shows a changed
  recommendation because of a human review rule" — that is the one thing to
  protect.
